<script lang="ts">
	...
	const currentErrorRate = $derived(
		realtimeStore.latestMetrics.error_rate ?? data.current.errorRate
	);
	const currentActiveUsers = $derived(
		realtimeStore.latestMetrics.active_users ?? data.current.activeUsers
	);
...
<div class="space-y-6">
	<div class="flex items-center gap-3">
		...
		{#if realtimeStore.connected}
			...
			>
				<span class="h-1.5 w-1.5 rounded-full" style="background-color: var(--color-success)"
				></span>
				Live
			</span>
		{/if}
