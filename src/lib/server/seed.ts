...
const PRODUCTS = ['Analytics Pro', 'Dashboard Suite', 'Data Connector', 'Report Builder', 'API Gateway'];
...
const CUSTOMERS = [
	...
	'Tyrell Corp', 'Weyland-Yutani', 'Massive Dynamic', 'Hooli', 'Pied Piper',
	'Aperture Science', 'Black Mesa', 'Wonka Industries', 'Dunder Mifflin', 'Sterling Cooper'
];
...
async function seed() {
	...
	for (let i = 0; i < salesData.length; i += 50) {
		db.insert(sales).values(salesData.slice(i, i + 50)).run();
	}
	...
	for (let i = 0; i < 30; i++) {
		for (let h = 0; h < 24; h++) {
			...
			for (const m of METRIC_NAMES) {
				...
				const value = Math.max(0, m.base + (Math.random() - 0.5) * m.variance + m.base * hourFactor);
				metricsData.push({
					name: m.name,
	...
	for (let i = 0; i < metricsData.length; i += 100) {
		db.insert(metrics).values(metricsData.slice(i, i + 100)).run();
	}
	...
	for (let i = 0; i < eventsData.length; i += 50) {
		db.insert(events).values(eventsData.slice(i, i + 50)).run();
	}
